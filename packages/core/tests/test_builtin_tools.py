...
async def test_write_then_view_round_trip(memory_tools):
    written = await memory_tools["memory:write"].handler(key="memory:user:ide", body="PyCharm")
    assert "revision" in written
    assert await memory_tools["memory:view"].handler(key="memory:user:ide") == "PyCharm"


async def test_list_only_returns_the_index(memory_tools):
    await memory_tools["memory:write"].handler(key="memory:user:ide", body="PyCharm")
    await memory_tools["memory:write"].handler(key="memory:agent:name", body="miniviki")
    listed = await memory_tools["memory:list"].handler()
    assert "memory:user:ide" in listed
    assert "PyCharm" not in listed


async def test_list_on_an_empty_store_says_so(memory_tools):
    assert await memory_tools["memory:list"].handler() == "the store is empty"


async def test_patch_edits_in_place(memory_tools):
    await memory_tools["memory:write"].handler(key="skill:ydpy", body="one\ntwo\n")
    await memory_tools["memory:patch"].handler(key="skill:ydpy", old_text="two", new_text="2")
    assert await memory_tools["memory:view"].handler(key="skill:ydpy") == "one\n2\n"


async def test_history_lists_revisions_newest_first(memory_tools):
    await memory_tools["memory:write"].handler(key="memory:user:ide", body="PyCharm")
    await memory_tools["memory:write"].handler(key="memory:user:ide", body="Neovim")
    listed = await memory_tools["memory:history"].handler(key="memory:user:ide")
...
async def test_diff_shows_the_change(memory_tools):
    first = await memory_tools["memory:write"].handler(key="memory:x", body="PyCharm\n")
    second = await memory_tools["memory:write"].handler(key="memory:x", body="Neovim\n")
    rev_a = first.rsplit(" ", 1)[-1]
    rev_b = second.rsplit(" ", 1)[-1]
    diff = await memory_tools["memory:diff"].handler(key="memory:x", rev_a=rev_a, rev_b=rev_b)
...
async def test_restore_brings_the_old_body_back_without_rewinding(memory_tools):
    first = await memory_tools["memory:write"].handler(key="memory:x", body="PyCharm")
    await memory_tools["memory:write"].handler(key="memory:x", body="Neovim")
    rev = first.rsplit(" ", 1)[-1]
    await memory_tools["memory:restore"].handler(key="memory:x", rev=rev)
    assert await memory_tools["memory:view"].handler(key="memory:x") == "PyCharm"
    assert len((await memory_tools["memory:history"].handler(key="memory:x")).splitlines()) == 3


async def test_delete_is_recoverable(memory_tools):
    await memory_tools["memory:write"].handler(key="memory:tmp", body="x")
    await memory_tools["memory:delete"].handler(key="memory:tmp")
    assert await memory_tools["memory:list"].handler() == "the store is empty"
    assert "memory:tmp" in await memory_tools["memory:history"].handler(key="memory:tmp")


async def test_search_finds_a_body_and_returns_a_snippet(memory_tools):
    await memory_tools["memory:write"].handler(
        key="memory:user:ide",
        body="the user prefers PyCharm"
    )
    await memory_tools["memory:write"].handler(key="memory:user:os", body="the user runs NixOS")
    hits = await memory_tools["memory:search"].handler(query="PyCharm")
...
async def test_search_reports_a_miss_honestly(memory_tools):
    assert "nothing matched" in await memory_tools["memory:search"].handler(query="unicorn")


async def test_search_respects_a_prefix(memory_tools):
    await memory_tools["memory:write"].handler(key="memory:user:ide", body="PyCharm")
    await memory_tools["memory:write"].handler(key="skill:ide", body="PyCharm tips")
    hits = await memory_tools["memory:search"].handler(query="PyCharm", prefix="skill")
...
async def test_body_cap_is_enforced_through_the_tools(tmp_path):
    ...
    await tools["memory:write"].handler(key="memory:one", body="12345678")
    with pytest.raises(KVLimitExceeded):
        await tools["memory:write"].handler(key="memory:one", body="x" * 9)


async def test_key_cap_is_enforced_through_the_tools(tmp_path):
    store = VersionedKVStore.open(tmp_path / "small", max_body_chars=8, max_keys=1)
    tools = {spec.name: spec for spec in build_memory_tools(store)}
    await tools["memory:write"].handler(key="memory:one", body="ok")
    with pytest.raises(KVLimitExceeded):
        await tools["memory:write"].handler(key="memory:two", body="ok")
...
async def test_missing_key_surfaces_as_an_error(tmp_path):
    ...
    with pytest.raises(KVKeyNotFound):
        await tools["memory:view"].handler(key="memory:nope")
